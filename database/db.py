...
def insert_verified_user(name, phone, rfid_id, token, chat_id):
    """Insert a verified user into PostgreSQL after Telegram verification."""
    query = """
        INSERT INTO users (rfid_id, name, phone, token, chat_id, state)
        VALUES (%s, %s, %s, %s, %s, 'OUTSIDE')
    """
    conn = None
    try:
        conn = get_connection()
        with conn.cursor() as cur:
            cur.execute(query, (rfid_id, name, phone, token, chat_id))
        conn.commit()
        logging.info(f"User Inserted Into Database: RFID: {rfid_id}, Name: {name}, Chat ID: {chat_id}")
    except Exception as e:
        logging.error(f"Error inserting verified user: {e}")
        raise
    finally:
        if conn:
            conn.close()
